  ...
  public partial class EventStoreGrpcTests
  {
    ...
    public async Task Append_Concurrency()
    {
      var clientOneRead = _client.ReadStreamAsync(
        Direction.Forwards, TestNoStream, StreamPosition.Start,
      ...
      var clientOneData = new EventData(Uuid.NewUuid(), EventType,
        Encoding.UTF8.GetBytes("{\"id\": \"1\" \"value\": \"clientOne\"}"));
      await _client.AppendToStreamAsync(TestNoStream, clientOneRevision,
        new List<EventData> { clientOneData });

      try
      {
        var clientTwoRead = _client.ReadStreamAsync(Direction.Forwards, TestNoStream, StreamPosition.Start);
        var clientTwoRevision = (await clientTwoRead.LastAsync()).Event.EventNumber.ToUInt64();

        var clientTwoData = new EventData(Uuid.NewUuid(), EventType,
          Encoding.UTF8.GetBytes("{\"id\": \"2\" \"value\": \"clientTwo\"}"));
        await _client.AppendToStreamAsync(TestNoStream, clientTwoRevision,
          new List<EventData> { clientTwoData });
      }
      catch (Exception ex)
      {
        Assert.IsType<WrongExpectedVersionException>(ex);
